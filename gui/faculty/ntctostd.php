<?php
	...
	{
		$std_id = trim(addslashes($_POST['std_id']));
		...
		{
			$sql="INSERT INTO teacher_std_notice (std_id,message,CreateBy,CreateDate) 
				  VALUES ('$std_id','$message','$CreateBy','$CreateDate')";
			$oResult=$oBasic->SqlQuery($sql);
			
			echo "<script>alert('Insert Notice Successfully');</script>";
			echo "<script>window.location='?Basic=NoticeToStudent'</script>";
		}
...
{
	echo '"'.$oResultMember->rows[$i]['roll'].'--'.$oResultMember->rows[$i]['fname'].' '.$oResultMember->rows[$i]['lname'].'"';
...
{
	var Name=document.getElementById('std_id').value;
	flag=true;
							...
							<form method="post" action="" enctype="multipart/form-data" id="registerform">
								<div class="box-body">
									...
									<div class="form-group">
									  <label>Student</label>
									  <input type="text" class="form-control" name="std_id" id="std_id" required="required" value="<?php echo ($id?$oResultUp->row['std_id']:''); ?>" onblur="LoadLoanInv()" />
									</div>
